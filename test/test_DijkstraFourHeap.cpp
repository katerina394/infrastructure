...
TEST(DijkstraFourHeap, throws_when_parametrs_are_incorrect) {
    vector < vector < pair<int, int> > > gr1;
    EXPECT_ANY_THROW(vector <int> res = DijkstraFourHeap(gr1, 1));
    vector < vector < pair<int, int> > > gr2(7);
    EXPECT_ANY_THROW(vector <int> res = DijkstraFourHeap(gr2, 8));
}
TEST(DijkstraFourHeap, work_right_for_graph_with_one_vertex) {
...
TEST(DijkstraFourHeap, work_right_for_disconnected_graph) {
